...
object PermutationManager {
    ...
    private val expandPermutationTable = arrayOf(
            32, 1, 2, 3, 4, 5,
            4, 5, 6, 7, 8, 9,
            8, 9, 10, 11, 12, 13,
            12, 13, 14, 15, 16, 17,
    ...
    private val initialReversedPermutationTable = arrayOf(
    ...
    private val keyInitialPermutationTable = arrayOf(
    ...
    private val encodeKeyTable = arrayOf(
            14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4,
            26, 8, 16, 7, 27, 20, 13, 2, 41, 52, 31, 37, 47, 55, 30, 40,
            51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32
    )

    private val sTables = arrayOf(
            arrayOf(
    ...
    private val shiftTable = arrayOf(1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1)

    fun extractSValue(iteration: Int, row: Int, column: Int) = sTables[iteration][row][column]

    fun performPPermutation(input: String): String {
        require(input.length == 32)
        return pTable.map { input[it - 1].toString() }.reduce { a, b -> a.plus(b) }
    }

    fun performInitialPermutation(input: String): String {
        require(input.length == 64) { throw IllegalArgumentException("Block must be 64 input!") }
        val chars = input.toCharArray()
        val sb = StringBuilder()
        initialPermutationTable
                .map { index -> chars[index - 1] }
                .forEach { sb.append(it) }
        return sb.toString()
    }

    fun performReverseInitialPermutation(input: String): String {
        require(input.length == 64) { throw IllegalArgumentException("Block must be 64 input!") }
        val chars = input.toCharArray()
        val sb = StringBuilder()
        initialReversedPermutationTable
                .map { index -> chars[index - 1] }
                .forEach { sb.append(it) }
        return sb.toString()
    }

    fun performExpandPermutation(bits: String): String {
        val chars = bits.toCharArray()
        val sb = StringBuilder()
        expandPermutationTable
                .map { index -> chars[index - 1] }
                .forEach { sb.append(it) }
        return sb.toString()
    }
    ...
    fun getInitialKeyPermutationValues(): Pair<List<Int>, List<Int>> {
        return keyInitialPermutationTable
                .slice(0 until keyInitialPermutationTable.size / 2) to
                keyInitialPermutationTable
                        .slice(keyInitialPermutationTable.size / 2 until keyInitialPermutationTable.size)
    }

    fun shiftKeyPartsLeft(left: List<Int>, iteration: Int): List<Int> {
        return shiftList(left, -shiftTable[iteration])
    }

    fun shiftKeyPartsRight(left: List<Int>, iteration: Int): List<Int> {
        return shiftList(left, shiftTable[iteration])
    }

    fun performEncodeKeyPermutation(key: String): String {
        require(key.length == 56)
        return encodeKeyTable.map { key[it - 1].toString() }.reduce { a, b -> a.plus(b) }.also {
            require(it.length == 48)
        }
    }

    fun <T> shiftList(input: List<T>, shift: Int): List<T> {
        Collections.rotate(input, shift)
        return input
    }
